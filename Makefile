...
$(hybrid): $(wildcard dir/$@/)
	find ./src/$@/ ./closed/src/$@/ -name *.tex | sort -n | awk '{printf "\\input{%s}\n", $$1}' > ./src/tmp/$@.tex;\
	
test:$(wildcard dir/src/)
	find ./src/bsp/ ./src/mdl/ ./src/ue/ ./src/lab/ ./src/exm/ ./src/skp/ ./closed/src/bsp/ ./closed/src/mdl/ ./closed/src/ue/ ./closed/src/lab/ ./closed/src/exm/ ./closed/src/skp/ -name *.tex -mmin -20 | sort -n| awk '{printf "\\input{%s}\n", $$1}' > ./src/tmp/test.tex;\
